...
final class BackgroundTaskManager {
    ...
    private func fetchMetrics() async -> (confidence: Double, risk: Double, profit: Double) {
        ...
        if let _ = getPortfolioStore(), let _ = getAINewsProvider() {
            ...
            //    await aiNewsProvider.refreshInsights(for: portfolioStore)
            //
            ...
            // let confidence = await aiNewsProvider.latestConfidenceChange(for: portfolioStore)
            // let risk = await portfolioStore.latestRiskChange()
            // let profit = await aiNewsProvider.latestProfitLikelihoodDelta(for: portfolioStore)
    ...
    private func getPortfolioStore() -> PortfolioStore? {
        // In a background task, we create a new instance to load data from storage.
        // This allows the task to work with the user's latest data.
        return PortfolioStore()
    }

    private func getAINewsProvider() -> AINewsProvider? {
        // Create a new AINewsProvider instance for the background task.
