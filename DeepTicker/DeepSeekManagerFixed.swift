...
class DeepSeekManagerFixed: ObservableObject {
    static let shared = DeepSeekManagerFixed()
    ...
    private func buildPredictionPrompt(for stock: Stock, with historicalData: [HistoricalDataPoint]) -> String {
        ...
        let dataString = recentData.map { point in
            "Date: \(DateFormatter.dsShortDate.string(from: point.date)), Close: $\(point.close), Volume: \(point.volume)"
        }.joined(separator: "\n")
...
private extension DateFormatter {
    static let dsShortDate: DateFormatter = {
